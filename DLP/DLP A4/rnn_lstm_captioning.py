...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        ...
        step_dx, step_dprev_h, step_dWx, step_dWh, step_db = rnn_step_backward(dnext_h, cache[t])
        dx[:, t, :] = step_dx
        dWx += step_dWx
        dWh += step_dWh
        db += step_db
        dprev_h = step_dprev_h
    dh0 = dprev_h
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.view(-1, x.shape[2]), y.view(-1), ignore_index=ignore_index, reduction='mean')
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # (1) Extract image features and project to initial hidden state
        image_features = self.image_encoder(images)  # (N, C, H, W)
        if self.cell_type == "attn":
            # For attention LSTM, project to (N, H, 4, 4)
            N = image_features.shape[0]
            A = self.feature_projection(image_features.view(N, -1, 16)).view(N, self.rnn.Wx.shape[1], 4, 4)
            h0 = None  # Not used for attention LSTM
        else:
            # For RNN/LSTM, project to (N, H)
            N = image_features.shape[0]
            image_features_flat = image_features.view(N, -1)
            h0 = self.feature_projection(image_features_flat)  # (N, H)
            A = None
        
        # (2) Embed captions
        word_embeddings = self.word_embedding(captions_in)  # (N, T, W)
        
        # (3) Run RNN/LSTM
        if self.cell_type == "attn":
            hidden_states = self.rnn(word_embeddings, A)  # (N, T, H)
        else:
            hidden_states = self.rnn(word_embeddings, h0)  # (N, T, H)
        
        # (4) Project to vocabulary
        vocab_scores = self.output_projection(hidden_states)  # (N, T, V)
        
        # (5) Compute loss
        loss = temporal_softmax_loss(vocab_scores, captions_out, ignore_index=self.ignore_index)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            ...
            image_features_flat = image_features.view(N, -1)
            h = self.feature_projection(image_features_flat)
            c = torch.zeros_like(h)
        else:  # rnn
            image_features_flat = image_features.view(N, -1)
            h = self.feature_projection(image_features_flat)
        
        word = self._start * images.new(N).long().fill_(1)
        
        for t in range(max_length):
            # (1) Embed current word
            word_embed = self.word_embedding(word)  # (N, W)
            
            # (2) RNN/LSTM step
            if self.cell_type == "lstm":
                h, c = self.rnn.step_forward(word_embed, h, c)
            elif self.cell_type == "attn":
                # For attention LSTM, compute attention
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        gates = torch.mm(x, self.Wx) + torch.mm(prev_h, self.Wh) + self.b
        H = prev_h.shape[1]
        i_gate = torch.sigmoid(gates[:, :H])
        f_gate = torch.sigmoid(gates[:, H:2*H])
        o_gate = torch.sigmoid(gates[:, 2*H:3*H])
        g_gate = torch.tanh(gates[:, 3*H:])
...
def dot_product_attention(prev_h, A):
    ...
    # Replace "pass" statement with your code
    # Reshape A to (N, H, 16)
    A_flat = A.view(N, H, 16)  # (N, H, 16)
    # Compute dot product: (N, H) @ (N, H, 16) -> (N, 16)
    ...
    # Compute weighted sum of A_flat
    attn = torch.bmm(attn_weights.unsqueeze(1), A_flat).squeeze(1)  # (N, H)
    # Reshape attn_weights back to (N, 4, 4)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        #######################################################################
        gates = torch.mm(x, self.Wx) + torch.mm(prev_h, self.Wh) + torch.mm(attn, self.Wattn) + self.b
        H = prev_h.shape[1]
        i_gate = torch.sigmoid(gates[:, :H])
        f_gate = torch.sigmoid(gates[:, H:2*H])
        o_gate = torch.sigmoid(gates[:, 2*H:3*H])
