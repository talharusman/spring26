...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for i in range(K):
            scores = torch.zeros(K)
            for j in range(K):
                scores[j] = torch.dot(query[n, i], key[n, j]) / math.sqrt(M)
            weights = F.softmax(scores, dim=0)
            out[n, i] = torch.mv(value[n].t(), weights)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(query_proj, key_proj, value_proj, mask)
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        c = math.sqrt(6.0 / (dim_in + dim_out))
        
        self.heads = nn.ModuleList([SelfAttention(dim_in, dim_head, dim_head) for _ in range(num_heads)])
        self.output_proj = nn.Linear(dim_out, dim_in)
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        y = (x - mean) / torch.sqrt(var + self.epsilon) * self.gamma + self.beta
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.multi_head_attention = MultiHeadAttention(num_heads, emb_dim, emb_dim)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    pos = torch.arange(K, dtype=torch.float32) / K
